...
static bool lua_module_initialize(shuso_t *S, shuso_module_t *module) {
  ...
  if(lua_istable(L, -1) {
    ...
    while(lua_next(L, -2)) {
      ...
      if(!shuso_event_initialize(S, &event[i], pub_event_name, data_type, cancelable)) {
        lua_pushboolean(L, 1);
...
static int Lua_shuso_module_event_publish(lua_State *L) {
  ...
  assert(module);
  lua_module_core_ctx_t   *ctx = shuso_core_context(shuso_t *S, module);
  assert(ctx);
...
static bool datatype_float_wrap(shuso_t *S, void *d, void *pd) {
  lua_State *L = S->lua.state;
  lua_pushnumber(L, (double *)pd);
  return true;
}
static bool datatype_float_unwrap(shuso_t *S, void **d, void *pd) {
  return false;
}
 
static bool datatype_int_wrap(shuso_t *S, void *d, void *pd) {
  lua_State *L = S->lua.state;
  lua_pushnumber(L, (double *)pd);
  return true;
}
static bool datatype_int_unwrap(shuso_t *S, void **d, void *pd) {
  return false;
}
 
 //TODO: DATATYPE WRAPPINGS should suck less!
...
bool shuso_register_lua_event_data_types(shuso_t *S) {
  ...
  map = (shuso_event_data_type_map_t ){
    .wrap = datatype_float_wrap,
    .unwrap = datatype_float_unwrap,
    .language = "Lua",
    .data_type = "float"
  };
  if(!shuso_register_event_data_type_mapping(S, &map, &shuso_lua_bridge_module, true)) {
  ...
  map = (shuso_event_data_type_map_t ){
    .wrap = datatype_int_wrap,
    .unwrap = datatype_int_unwrap,
    .language = "Lua",
    .data_type = "integer"
  };
  if(!shuso_register_event_data_type_mapping(S, &map, &shuso_lua_bridge_module, true)) {
  ...
  map = (shuso_event_data_type_map_t ){
    .wrap = datatype_bool_wrap,
    .unwrap = datatype_bool_unwrap,
    .language = "Lua",
    .data_type = "boolean"
  };
  if(!shuso_register_event_data_type_mapping(S, &map, &shuso_lua_bridge_module, true)) {
