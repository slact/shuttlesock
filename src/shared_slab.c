...
#define ngx_slab_slots(pool)                                                  \
    (shuso_slab_page_t *) ((u_char *) (pool) + sizeof(shuso_slab_pool_t))

#define ngx_slab_page_type(page)   ((page)->prev & NGX_SLAB_PAGE_MASK)

#define ngx_slab_page_prev(page)                                              \
    (shuso_slab_page_t *) ((page)->prev & ~NGX_SLAB_PAGE_MASK)
...
static shuso_slab_page_t *ngx_slab_alloc_pages(shuso_slab_pool_t *pool,
    ngx_uint_t pages);
static void ngx_slab_free_pages(shuso_slab_pool_t *pool, shuso_slab_page_t *page,
    ngx_uint_t pages);
static void ngx_slab_error(shuso_slab_pool_t *pool, ngx_uint_t level,
    char *text);

#define ngx_pagesize shuso_system.page_size
#define ngx_pagesize_shift shuso_system.page_size_shift

#define ngx_slab_max_size shared_slab_max_size
#define ngx_slab_exact_size shared_slab_exact_size
#define ngx_slab_exact_shift shared_slab_exact_shift

uintptr_t shared_slab_max_size = 0;
...
void shuso_shared_slab_sizes_init(void) {
  ngx_uint_t  n;
  if(!shared_slab_initialized) {
    ngx_slab_max_size = ngx_pagesize / 2;
    ngx_slab_exact_size = ngx_pagesize / (8 * sizeof(uintptr_t));
    for (n = ngx_slab_exact_size; n >>= 1; ngx_slab_exact_shift++) {
        /* void */
    }
    shared_slab_initialized = true;
  }
}


void ngx_slab_init(shuso_slab_pool_t *pool) {
  u_char           *p;
  size_t            size;
  ngx_int_t         m;
  ngx_uint_t        i, n, pages;
  shuso_slab_page_t  *slots, *page;
  ...
  n = shuso_system.page_size_shift - pool->min_shift;
  ...
  p += n * sizeof(shuso_slab_page_t);

  pool->stats = (shuso_slab_stat_t *) p;
  ngx_memzero(pool->stats, n * sizeof(shuso_slab_stat_t));

  p += n * sizeof(shuso_slab_stat_t);

  size -= n * (sizeof(shuso_slab_page_t) + sizeof(shuso_slab_stat_t));

  pages = (ngx_uint_t) (size / (ngx_pagesize + sizeof(shuso_slab_page_t)));

  pool->pages = (shuso_slab_page_t *) p;
  ngx_memzero(pool->pages, pages * sizeof(shuso_slab_page_t));
  ...
  pool->start = ngx_align_ptr(p + pages * sizeof(shuso_slab_page_t), ngx_pagesize);
...
void *
ngx_slab_alloc(shuso_slab_pool_t *pool, size_t size)
{
    void  *p;
...
void *
ngx_slab_alloc_locked(shuso_slab_pool_t *pool, size_t size)
{
    size_t            s;
    uintptr_t         p, m, mask, *bitmap;
    ngx_uint_t        i, n, slot, shift, map;
    shuso_slab_page_t  *page, *prev, *slots;
...
void *
ngx_slab_calloc(shuso_slab_pool_t *pool, size_t size)
{
    void  *p;
...
void *
ngx_slab_calloc_locked(shuso_slab_pool_t *pool, size_t size)
{
    void  *p;
...
void
ngx_slab_free(shuso_slab_pool_t *pool, void *p)
{
    ngx_shmtx_lock(&pool->mutex);
...
void
ngx_slab_free_locked(shuso_slab_pool_t *pool, void *p)
{
    size_t            size;
...
static shuso_slab_page_t *
ngx_slab_alloc_pages(shuso_slab_pool_t *pool, ngx_uint_t pages)
{
    shuso_slab_page_t  *page, *p;
...
static void
ngx_slab_free_pages(shuso_slab_pool_t *pool, shuso_slab_page_t *page,
    ngx_uint_t pages)
{
    shuso_slab_page_t  *prev, *join;
    ...
    if (pages) {
        ngx_memzero(&page[1], pages * sizeof(shuso_slab_page_t));
    }
